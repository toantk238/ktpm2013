...
def MainRange():
    ...
    while i< len(RangeVariable):
        ...
        '''
        array_max = temp.array_max
        array_min = temp.array_min
        print 'min '+repr(i)+':'
        for x in xrange(0, len(array_min)):
            print array_min[x]   
        print 'max '+repr(i)+':'
        for x in xrange(0, len(array_max)):
            print array_max[x]
        ''' 
        i+=1
        
    # check range     
    #for i in xrange( 0, len(RangeVariable)) :
    #    temp = RangeVariable[i]
    arr1 = [5,3,20,16]  
    arr2 = [8,4,70,18]  
    sortTwoArray(arr1, arr2)    
    #print checkTwoArray(arr1, arr2)   
